...
Operation::Operation(DbToCode &dt, const SorcePar &spar)
{
	...
	this->dtcode = &dt;
	if (spar.tested)
		cameraApt = createCamera(this->dtcode->getSoftCfg().cameraType);
...
void Operation::star(bool c, int a, char *v[])
{
	...
	{
		auto img = createDetector(this->dtcode->getSoftCfg().detectorType);
		img->imgW = spar.imgW;
		img->imgH = spar.imgH;
		...
		inferstore->inferForeverToStr(
			[this](std::vector<std::string> sqls) {
				for (auto& valStr : sqls) {
					if (this->dtcode->getSoftCfg().saveDb)
						this->dtcode->combDecValStr(valStr);
					if (this->dtcode->getSoftCfg().sendAlert)
